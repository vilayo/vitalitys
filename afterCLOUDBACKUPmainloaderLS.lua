...
-- Supported production modules: The Tower, Apocalypse Rising 2, Murder Mystery 2, Flick, KAT (configure ID below)
...
local function buildManifestFromLiveState(state)
    ...
    for gameId, profile in pairs(SupportedGames) do
        local liveId = normalizeLiveModuleId(profile.LiveModuleId or profile.ModuleId)
        local live = modules[liveId]

        if type(live) == "table" then
            local statusName = tostring(live.status or profile.State or "testing"):lower()
            ...
            games[gameId] = {
                ModuleId = profile.ModuleId,
                LiveModuleId = liveId,
                Name = live.name or profile.Name,
                Version = live.version or profile.Version,
                ModuleURL = (type(live.url) == "string" and live.url ~= "") and live.url or profile.Module,
                State = statusName,
            ...
            games[gameId] = StatusGames[gameId]
        end
    end
...
local function handleLivePacket(rawMessage)
    ...
    if packet.type == "module.updated" then
        ...
        if packet.module and type(packet.moduleState) == "table" then
            VitalityLive.State.modules[tostring(packet.module)] = packet.moduleState
        end
...
local function registerLiveOwnerCommands(window)
    ...
    window:RegisterCommand({
        ...
        Callback = function(args)
            ...
            if not VALID_LIVE_MODULES[moduleId] then return window:Notify({Title = "Message game", Content = "Unknown module: " .. moduleId, Type = "Error", Duration = 4}) end
            local ok, result, err = liveOwnerRequest("POST", "/owner/broadcast", {
    ...
    window:RegisterCommand({
        ...
        Callback = function(args)
            ...
            if not VALID_LIVE_MODULES[moduleId] then return window:Notify({Title = "Status", Content = "Unknown module: " .. moduleId, Type = "Error", Duration = 4}) end
    ...
    window:RegisterCommand({
        ...
        Callback = function(args)
            ...
            local moduleId = normalizeLiveModuleId(table.remove(args, 1))
            if not VALID_LIVE_MODULES[moduleId] then return window:Notify({Title = "Reason", Content = "Unknown module: " .. moduleId, Type = "Error", Duration = 4}) end
    ...
    local function setEnabled(args, enabled)
        ...
        if not VALID_LIVE_MODULES[moduleId] then return window:Notify({Title = "Module control", Content = "Unknown module: " .. moduleId, Type = "Error", Duration = 4}) end
    ...
    window:RegisterCommand({
        ...
        Callback = function(args)
            ...
            local moduleId = normalizeLiveModuleId(table.remove(args, 1))
            if not VALID_LIVE_MODULES[moduleId] then return window:Notify({Title = "Module version", Content = "Unknown module: " .. moduleId, Type = "Error", Duration = 4}) end
...
local function bootstrapLivePolicy()
    ...
    local modules = type(response.state.modules) == "table" and response.state.modules or {}
    for gameId, profile in pairs(SupportedGames) do
        local liveId = normalizeLiveModuleId(profile.LiveModuleId or profile.ModuleId)
        local live = modules[liveId]
        if type(live) == "table" then
            local registered = NovaField.Router and NovaField.Router.Games and NovaField.Router.Games[gameId]
            local enabled = live.enabled ~= false
            profile.Enabled = enabled
            if registered then registered.Enabled = enabled end

            if type(live.url) == "string" and live.url:match("^https?://") then
                profile.Module = live.url
                if registered then registered.Module = live.url end
            end

            if type(live.version) == "string" and live.version ~= "" then
                profile.Version = live.version
                if registered then registered.Version = live.version end
            end

            local state = tostring(live.status or ""):lower()
            local definition = LOCAL_STATUS_DEFINITIONS[state]
            if definition then
                profile.State = state
                profile.Status = definition.LegacyLevel
                profile.Label = definition.Label
                if registered then
                    registered.State = state
                    registered.Status = definition.LegacyLevel
                    registered.Label = definition.Label
                end
            end
        end
    end
