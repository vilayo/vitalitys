...
Library.Themes = {
    Dark = {
        ...
        Warning = Color3.fromRGB(243, 194, 82),
    ...
    Light = {
        ...
        Warning = Color3.fromRGB(202, 142, 33),
...
local THEME_ROLE_PRIORITY = {
    ...
    "ButtonText", "ButtonOutline", "Success", "Danger",
}
...
local function applyButtonGradient(guiObject)
    local gradient = create("UIGradient", {
        Name = "NovaButtonGradient",
...
local function createVectorIcon(parent, name)
    ...
    elseif name == "vitality" or name == "pulse" then
        lineIcon(holder, UDim2.fromOffset(1, 10), UDim2.fromOffset(5, 2), 0, Theme.Text, 1)
        lineIcon(holder, UDim2.fromOffset(5, 8), UDim2.fromOffset(6, 2), -55, Theme.Text, 1)
        lineIcon(holder, UDim2.fromOffset(9, 9), UDim2.fromOffset(8, 2), 67, Theme.Text, 1)
        lineIcon(holder, UDim2.fromOffset(15, 10), UDim2.fromOffset(6, 2), 0, Theme.Text, 1)
...
function Library:Notify(data)
    ...
    if window then
        window:_playSound("Notification")
    end
    ...
    corner(card, 7)
    stroke(card, data.Success == false and Theme.Danger or Theme.Success, 1, 0.15)
    local cardScale = create("UIScale", {Parent = card, Scale = 0.97})

    local icon = makeText(card, data.Success == false and "!" or "✓", 16, Theme.Background, Enum.Font.GothamBold, Enum.TextXAlignment.Center)
    icon.BackgroundTransparency = 0
    setThemeRole(icon, "BackgroundColor3", data.Success == false and "Danger" or "Success")
    icon.Position = UDim2.fromOffset(16, 19)
    ...
    body.Position = UDim2.fromOffset(66, 36)
    body.Size = UDim2.new(1, -105, 0, 20)
    body.ZIndex = 302

    local close = makeText(card, "×", 22, Theme.Text, Enum.Font.Gotham, Enum.TextXAlignment.Center)
    close.Position = UDim2.new(1, -34, 0, 18)
    ...
    local progress = create("Frame", {
        ...
        Size = UDim2.fromScale(1, 1),
        BackgroundColor3 = data.Success == false and Theme.Danger or Theme.Success,
        BorderSizePixel = 0,
        ZIndex = 304,
    })
    setThemeRole(progress, "BackgroundColor3", data.Success == false and "Danger" or "Success")
    ...
    if window then
        window:_tween(card, 0.30, {GroupTransparency = 0, Size = UDim2.new(1, 0, 0, 74)})
        window:_tween(cardScale, 0.30, {Scale = 1})
    else
        tween(card, 0.30, {GroupTransparency = 0, Size = UDim2.new(1, 0, 0, 74)})
        tween(cardScale, 0.30, {Scale = 1})
    end
    local lifetime = math.max(tonumber(data.Duration) or 4, 0.5)
...
function WindowMethods:LoadConfiguration(fireCallbacks)
    ...
    for flag, value in pairs(self.ConfigData or {}) do
        local object = self.FlagObjects[flag]
        if object and object.Set then object:Set(value, fireCallbacks == true) end
    end
    return true
end
...
function WindowMethods:_refreshTabStyles()
    ...
    for _, other in ipairs(self.Tabs) do
        ...
        setThemeRole(other.NavButton, "BackgroundColor3", selected and "AccentSoft" or "Surface")
...
function WindowMethods:_applyThemePalette(newPalette, themeName)
    ...
    if self.Gui then
        for _, object in ipairs(self.Gui:GetDescendants()) do
            ...
            if object:IsA("UIGradient") and object.Name == "NovaButtonGradient" then
...
local SCRIPT_STATUS_STATES = {
    red = {Level = "red", Role = "Danger", Label = "Not working"},
    error = {Level = "red", Role = "Danger", Label = "Not working"},
    offline = {Level = "red", Role = "Danger", Label = "Not working"},
    yellow = {Level = "yellow", Role = "Warning", Label = "Partially working"},
    partial = {Level = "yellow", Role = "Warning", Label = "Partially working"},
    loading = {Level = "yellow", Role = "Warning", Label = "Partially working"},
    green = {Level = "green", Role = "Success", Label = "Fully working"},
    working = {Level = "green", Role = "Success", Label = "Fully working"},
    ready = {Level = "green", Role = "Success", Label = "Fully working"},
}
...
function WindowMethods:_refreshStatusChipLayout()
    ...
    if self.KeyTimeLabel.Visible then
        self.StatusLabel.Position = UDim2.fromOffset(31, 3)
        self.StatusLabel.Size = UDim2.new(1, -40, 0, 15)
        self.KeyTimeLabel.Position = UDim2.fromOffset(31, 17)
        self.KeyTimeLabel.Size = UDim2.new(1, -40, 0, 13)
    else
        self.StatusLabel.Position = UDim2.fromOffset(31, 0)
        self.StatusLabel.Size = UDim2.new(1, -40, 1, 0)
    end
end

function WindowMethods:SetScriptStatus(status, detail)
    if type(status) == "table" then
        detail = status.Detail or status.Label or detail
        status = status.Level or status.Status
    end
    local state = SCRIPT_STATUS_STATES[tostring(status or "yellow"):lower()] or SCRIPT_STATUS_STATES.yellow
    self.ScriptStatus = {
        Level = state.Level,
        Role = state.Role,
    ...
    if self.StatusDot then
        setThemeRole(self.StatusDot, "BackgroundColor3", state.Role)
        self.StatusDot.BackgroundColor3 = Theme[state.Role]
    end
    if self.StatusGlow then
        setThemeRole(self.StatusGlow, "BackgroundColor3", state.Role)
        self.StatusGlow.BackgroundColor3 = Theme[state.Role]
    end
...
function WindowMethods:GetScriptStatus()
    local status = self.ScriptStatus or SCRIPT_STATUS_STATES.yellow
...
function WindowMethods:GetStatusControlInfo()
    local copy = {}
    for key, value in pairs(self.RemoteStatusInfo or {}) do copy[key] = value end
    return copy
end
...
function WindowMethods:RefreshStatusControl()
    ...
    local applied = false
    if type(config.LocalManifest) == "table" then
        local ok = self:ApplyStatusManifest(config.LocalManifest, "local")
        applied = ok == true
    end

    local url = tostring(config.URL or ""):gsub("^%s+", ""):gsub("%s+$", "")
    if url == "" then return applied, applied and "local" or "no_url" end

    local fetcher = config.Fetcher
    local ok, body
    if type(fetcher) == "function" then
        ok, body = pcall(fetcher, url, self)
    else
    ...
    if not ok or body == nil then
        ...
        if not applied and type(config.FailureStatus) == "table" then
            self:SetScriptStatus(config.FailureStatus.Status or config.FailureStatus.Level or "yellow", config.FailureStatus.Label or "Status unavailable")
        end
...
function WindowMethods:SetVisible(visible, immediate)
    ...
    if visible then
        ...
        main.Size =
            self._minimized
            and (self._collapsedSize or UDim2.fromOffset(880, 48))
            or (self._expandedSize or UDim2.fromOffset(880, 560))
...
function WindowMethods:SetMinimized(minimized, immediate)
    ...
    local expanded =
        self._expandedSize
        or UDim2.fromOffset(880, 560)

    local collapsed =
        self._collapsedSize
        or UDim2.fromOffset(880, 48)
...
function WindowMethods:OpenPersonalization()
    for _, tab in ipairs(self.Tabs or {}) do
        local lowered =
            tostring(tab.Name or ""):lower()

        if lowered == "personalization"
            or lowered == "appearance" then
...
function WindowMethods:CreateTab(name, image)
    ...
    corner(nav, 7)
    local navStroke = stroke(nav, Theme.AccentVisible, 1, 0.46)
    ...
    -- Dedicated non-CanvasGroup clip wrapper. This is the authoritative page
    -- boundary; anything taller than this frame must be reached by scrolling.
    ...
    -- Regular Frames are intentional here. CanvasGroup creates a separate
    -- composition layer on Roblox and can visually bypass ancestor clipping on
    -- some clients. Keeping the scrolling content as Frames guarantees that the
    -- ScrollingFrame is the hard visual boundary.
    ...
    local function recalc()
        ...
        columns.Size = UDim2.new(1, -10, 0, h)
        left.Size = UDim2.new(0.5, -6, 0, h)
        right.Size = UDim2.new(0.5, -6, 0, h)
...
function TabMethods:CreateSection(name, subtitleOrTitleOnly, column)
    ...
    local headerHeight = titleOnly and 42 or (subtitle and 68 or 54)
    ...
    if sectionData.Icon then
        icon = createIcon(header, sectionData.Icon, "info")
    ...
    local heading = makeText(header, section.Name, 15, Theme.Text, Enum.Font.GothamSemibold)
    heading.TextXAlignment = Enum.TextXAlignment.Left

    if subtitle then
        heading.Position = UDim2.fromOffset(headingX, 10)
        heading.Size = UDim2.new(1, -(headingX + 16), 0, 25)
    else
        heading.AnchorPoint = Vector2.new(0, 0.5)
        heading.Position = UDim2.new(0, headingX, 0.5, 0)
        heading.Size = UDim2.new(1, -(headingX + 16), 0, 28)
    end

    local sub
    if subtitle then
        sub = makeText(header, subtitle, 11, Theme.Muted, Enum.Font.Gotham)
...
local function rowText(row, name, info, rightWidth)
    local right = rightWidth or 140
    local nameLabel = makeText(row, name or "Control", 13, Theme.Text, Enum.Font.GothamMedium)
    nameLabel.Position = UDim2.fromOffset(16, info and 8 or 0)
    nameLabel.Size = UDim2.new(1, -(right + 38), info and 0 or 1, info and 20 or 0)
    if not info then nameLabel.TextYAlignment = Enum.TextYAlignment.Center end

    local infoLabel
    if info then
        infoLabel = makeText(row, info, 11, Theme.Muted, Enum.Font.Gotham)
        infoLabel.Position = UDim2.fromOffset(16, 26)
        infoLabel.Size = UDim2.new(1, -(right + 38), 0, 18)
        infoLabel.TextWrapped = true
        infoLabel.TextYAlignment = Enum.TextYAlignment.Top
    end
    return nameLabel, infoLabel
end
...
function TabMethods:CreateButton(data)
    ...
    local row = section:_row(64)
    rowText(row, data.Name or "Button", data.Info, 106)
    ...
    corner(button, 5)
    local buttonOutline = stroke(button, Theme.ButtonOutline, 1, 0.20)
    applyButtonGradient(button)
    bindHover(button, function() return Theme.Button end, function() return Theme.ButtonHover end)

    local object = {}
    local function renderButtonState(locked)
        locked = locked == true or object.Locked == true
        button.Active = not locked
        setThemeRole(button, "BackgroundColor3", "Button")
        setThemeRole(button, "TextColor3", locked and "Muted" or "ButtonText")
    ...
    button.MouseButton1Click:Connect(function()
        ...
        window:_tween(button, 0.07, {Size = UDim2.fromOffset(86, 34)})
        task.delay(window:_motionDuration(0.08), function()
            if button.Parent then window:_tween(button, 0.10, {Size = UDim2.fromOffset(90, 36)}) end
        end)
...
function TabMethods:CreateToggle(data)
    ...
    local section = self:_sectionFrom(data)
    local row = section:_row(64)
    rowText(row, data.Name or "Toggle", data.Info, 88)
    ...
    local shell = create("TextButton", {
        ...
        BackgroundColor3 = value and Theme.AccentVisible or Theme.Disabled,
        BorderSizePixel = 0,
        Text = "",
        AutoButtonColor = false,
    })
    corner(shell, 14)
    local toggleStroke = stroke(shell, value and Theme.AccentVisible or Theme.Border, 1, 0.12)
    ...
    local function render(animate)
        local shellRole = object.Value and "AccentVisible" or "Disabled"
        local knobRole = object.Value and "AccentText" or "Text"
        local shellProps = {BackgroundColor3 = Theme[shellRole]}
        local knobProps = {Position = object.Value and UDim2.fromOffset(27, 3) or UDim2.fromOffset(3, 3)}
        setThemeRole(shell, "BackgroundColor3", shellRole)
    ...
    shell.MouseButton1Click:Connect(function()
        ...
        if behavior == "sound" or behavior == "sounds" then
            -- Let enabling Sounds prove itself immediately.
    ...
    -- Behavior toggles control the actual library feature, not just their callback.
    if behavior == "notifications" then
        window:SetNotificationsEnabled(object.Value)
    elseif behavior == "sound" or behavior == "sounds" then
        window:SetSoundsEnabled(object.Value)
...
function TabMethods:CreateSlider(data)
    ...
    local window = self.Window
    local section = self:_sectionFrom(data)
    local row = section:_row(76)
    rowText(row, data.Name or "Slider", data.Info, 206)
    ...
    local value = math.clamp(saved, min, max)
    ...
    local suffixWidth = suffixText ~= "" and 20 or 0
    local boxWidth = 52
    local gap = 14
    ...
    local box = create("TextBox", {
        ...
        Size = UDim2.fromOffset(boxWidth, 34),
    ...
    if suffixText ~= "" then
        ...
        suffixLabel.Size = UDim2.fromOffset(suffixWidth - 2, 24)
    end
    ...
    local function render()
        ...
        setThemeRole(fill, "BackgroundColor3", "AccentVisible")
        ...
        box.Text = tostring(object.Value)
    end

    function object:Set(newValue, fire)
        newValue = tonumber(newValue) or object.Value
        newValue = math.clamp(roundTo(newValue, increment), min, max)
        if math.abs(newValue - math.floor(newValue)) < 0.00001 then
...
function TabMethods:CreateInput(data)
    ...
    local row = section:_row(hasCounter and 82 or 62)
    rowText(row, data.Name or "Input", data.Info, 206)

    local initial = tostring(window:_saved(data.Flag, data.CurrentValue or ""))
    local inputBox = create("TextBox", {
        Parent = row,
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -16, 0.5, hasCounter and -5 or 0),
...
function TabMethods:CreateDropdown(data)
    ...
    local window = self.Window
    local section = self:_sectionFrom(data)
    local row = section:_row(66)
    rowText(row, data.Name or "Dropdown", data.Info, 190)
    ...
    local value = window:_saved(data.Flag, fallback)
    ...
    corner(button, 5)
    local buttonStroke = stroke(button, Theme.Border, 1, 0.08)
    ...
    local function openPopup()
        ...
        local popupHeight = math.min(#object.Options * 42 + 8, 220)
        ...
        local list = create("ScrollingFrame", {
            ...
            CanvasSize = UDim2.fromOffset(0, #object.Options * 42),
            ScrollBarThickness = #object.Options * 42 > popupHeight and 3 or 0,
            ScrollBarImageColor3 = Theme.Disabled,
            ZIndex = 221,
        })
        create("UIListLayout", {
...
function TabMethods:CreateColorPicker(data)
    ...
    local section = self:_sectionFrom(data)
    local row = section:_row(66)
    rowText(row, data.Name or "Color Picker", data.Info, 190)
    ...
    hexLabel.Position = UDim2.fromOffset(46, 0)
    ...
    if autoAccentPicker and data.Flag and window.ConfigData[data.Flag] ~= nil then
        window:SetThemeAccent(object.Value)
    elseif autoButtonPicker and data.Flag and window.ConfigData[data.Flag] ~= nil then
...
function TabMethods:CreateLabel(text, sectionParent)
    ...
    local row = section:_row(42, true)
    local label = makeText(row, tostring(text or ""), 11, Theme.Muted, Enum.Font.Gotham)
    label.Position = UDim2.fromOffset(16, 4)
    label.Size = UDim2.new(1, -32, 1, -8)
    label.TextWrapped = true
    local object = {}
    function object:Set(newText) label.Text = tostring(newText or "") end
    object.Instance = row
    return object
end
...
function TabMethods:CreateParagraph(data, sectionParent)
    ...
    local row = section:_row(58, true)
    local title = makeText(row, data.Title or "Paragraph", 13, Theme.Text, Enum.Font.GothamSemibold)
    title.Position = UDim2.fromOffset(16, 8)
    title.Size = UDim2.new(1, -32, 0, 20)
    local body = makeText(row, data.Content or "", 11, Theme.Muted, Enum.Font.Gotham)
    body.Position = UDim2.fromOffset(16, 29)
    body.Size = UDim2.new(1, -32, 0, 22)
    body.TextWrapped = true
    body.TextYAlignment = Enum.TextYAlignment.Top
    local object = {}
    function object:Set(newData)
        if type(newData) == "table" then
            if newData.Title then title.Text = newData.Title end
            if newData.Content then body.Text = newData.Content end
        end
    end
    object.Instance = row
    return object
...
function TabMethods:CreateKeybind(data)
    ...
    local window = self.Window
    local section = self:_sectionFrom(data)
    local row = section:_row(58)
    rowText(row, data.Name or "Keybind", data.Info, 190)
...
function TabMethods:CreateStatus(data)
    ...
    local section = self:_sectionFrom(data)
    local row = section:_row(58)
    rowText(row, data.Name or "Status", data.Info, 120)
...
function WindowMethods:ShowLoading(data)
    ...
    local card = create("Frame", {
        ...
        Size = UDim2.fromOffset(320, 274),
        BackgroundColor3 = Theme.Surface,
        BorderSizePixel = 0,
        ZIndex = 381,
    })
    corner(card, 10)
    stroke(card, Theme.Border, 1, 0.38)
    ...
    corner(headerIcon, 9)
    stroke(headerIcon, Theme.AccentVisible, 2, 0.05)
    headerIcon.Position = UDim2.fromOffset(16, 13)
    headerIcon.Size = UDim2.fromOffset(20, 20)
    headerIcon.ZIndex = 382
    local header = makeText(card, "vitality", 10, Theme.Muted, Enum.Font.GothamMedium)
    header.Position = UDim2.fromOffset(46, 9)
    ...
    corner(track, 29)
    stroke(track, Theme.BorderSoft, 4, 0.28)
    for index = 1, 12 do
    ...
    local logo = createIcon(card, data.Icon or self.Settings.Icon or "vitality", "vitality")
    logo.AnchorPoint = Vector2.new(0.5, 0.5)
    logo.Position = UDim2.new(0.5, 0, 0, 102)
    logo.Size = UDim2.fromOffset(24, 24)
    logo.ZIndex = 385
    setIconColor(logo, Theme.Text)
    ...
    title.Position = UDim2.fromOffset(20, 151)
    ...
    status.Position = UDim2.fromOffset(20, 181)
    ...
    percent.Position = UDim2.new(1, -50, 0, 217)
    ...
    function controller:SetProgress(value, animate)
        ...
        if animate == false then
            ...
            selfWindow:_tween(progressFill, 0.42, goal, Enum.EasingStyle.Quint)
        end
    end
...
local function buildKeySystem(window)
    ...
    bindHover(verify, function() return Theme.Button end, function() return Theme.ButtonHover end)
    ...
    local function rejectKey(message)
        ...
        errorText.Text = tostring(message or "●  Invalid key")
    ...
    local function tryRemoteKey()
        ...
        task.spawn(function()
            ...
            if not accepted then
                ...
                rejectKey(message or "●  Invalid key")
...
function Library:LoadDetectedGame(window, options)
    ...
    local profile = detection.Profile or {}
    if window then
        local status = profile.Status or profile.Level or options.ReadyStatus or "green"
...
function Library:CreateWindow(settings)
    ...
    window.ScriptStatus = {Level = "yellow", Role = "Warning", Label = "Partially working"}
    ...
    local requestedTheme = tostring(settings.Theme or "Dark")
    ...
    if typeof(settings.AccentColor) == "Color3" then
        applyAccentPalette(initialPalette, settings.AccentColor)
    else
        applyAccentPalette(initialPalette, initialPalette.Accent)
    end
    if typeof(settings.ButtonColor) == "Color3" then
        applyButtonPalette(initialPalette, settings.ButtonColor)
    else
    ...
    window._expandedSize = UDim2.fromOffset(880, 560)
    window._collapsedSize = UDim2.fromOffset(880, 48)
    ...
    logo.Position=UDim2.fromOffset(17,14); logo.Size=UDim2.fromOffset(20,20)

    local title=makeText(topbar, settings.Name or "vitality's hub", 15, Theme.Text, Enum.Font.GothamSemibold)
    title.Position=UDim2.fromOffset(49,0); title.Size=UDim2.new(1,-365,1,0)
    ...
    local statusChip=create("Frame",{
        Parent=topbar,AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-116,0.5,0),Size=UDim2.fromOffset(200,34),
        BackgroundColor3=Theme.Surface3,BackgroundTransparency=0.04,BorderSizePixel=0,
    })
    corner(statusChip,8); stroke(statusChip,Theme.Border,1,0.28)
    local statusGlow=create("Frame",{
        Parent=statusChip,AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0,16,0.5,0),Size=UDim2.fromOffset(17,17),
        BackgroundColor3=Theme.Warning,BackgroundTransparency=0.72,BorderSizePixel=0,
    })
    corner(statusGlow,9)
    local statusDot=create("Frame",{
        Parent=statusChip,AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0,16,0.5,0),Size=UDim2.fromOffset(9,9),
        BackgroundColor3=Theme.Warning,BorderSizePixel=0,
    })
    corner(statusDot,5); stroke(statusDot,Theme.Background,1.5,0.02)
    local statusLabel=makeText(statusChip,"Partially working",11,Theme.Text,Enum.Font.GothamSemibold)
    statusLabel.TextTruncate=Enum.TextTruncate.AtEnd
    statusLabel.Position=UDim2.fromOffset(31,0); statusLabel.Size=UDim2.new(1,-40,1,0)
    local keyTimeLabel=makeText(statusChip,"",8,Theme.Muted,Enum.Font.GothamMedium)
    ...
    corner(personalizationButton,6)
    ...
    bindHover(
        personalizationButton,
        function() return Theme.AccentSoft end,
        function() return Theme.AccentVisible end
    )
    ...
    local minimize=create("TextButton",{
        Parent=topbar,AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-46,0.5,0),Size=UDim2.fromOffset(26,26),
        BackgroundColor3=Theme.Surface3,BorderSizePixel=0,Text="−",TextColor3=Theme.Text,TextSize=18,Font=Enum.Font.Gotham,AutoButtonColor=false,
    ...
    local close=create("TextButton",{
        Parent=topbar,AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-14,0.5,0),Size=UDim2.fromOffset(26,26),
        BackgroundColor3=Theme.Surface3,BorderSizePixel=0,Text="×",TextColor3=Theme.Text,TextSize=20,Font=Enum.Font.Gotham,AutoButtonColor=false,
    ...
    local nav=create("CanvasGroup",{
        Parent=bodyViewport,BackgroundColor3=Theme.Surface,GroupTransparency=0,BorderSizePixel=0,Position=UDim2.fromOffset(0,0),Size=UDim2.new(0,146,1,0),
    ...
    local navList=create("Frame",{Parent=nav,BackgroundTransparency=1,Position=UDim2.fromOffset(9,12),Size=UDim2.new(1,-18,1,-82)})
    ...
    local content=create("Frame",{
        Parent=bodyViewport,BackgroundTransparency=1,BorderSizePixel=0,Position=UDim2.fromOffset(146,0),Size=UDim2.new(1,-146,1,-1),
    ...
    local function updateScale()
        ...
        uiScale.Scale=math.clamp(math.min(vp.X/960,vp.Y/630),0.86,1)
    end
    updateScale()
