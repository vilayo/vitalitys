...
function WindowMethods:_refreshTabStyles()
    ...
    for _, other in ipairs(self.Tabs) do
        ...
        -- UIGradient multiplies against BackgroundColor3. Active tab gradients are
        -- authored on a white base by RefreshNavGradient, so never recolor that base
        -- after refreshing it. This keeps first-click and repeat-click rendering equal.
        if selected then
...
function WindowMethods:_setActiveTab(tab)
    ...
    for _, other in ipairs(self.Tabs) do
        ...
        -- Only tween transparency here. Tweening BackgroundColor3 recolors the
        -- UIGradient multiplication base and is what made the first click darker.
...
function WindowMethods:_buildPersonalization(tab)
    ...
    -- Game modules may move the notification controls to a visible Settings tab.
    -- When they do, skip the duplicate Personalization copy and leave everything
    -- else in this builder unchanged.
    if tab._VitalityNotificationsMoved ~= true then
