...
function WindowMethods:_refreshTabStyles()
    ...
    for _, other in ipairs(self.Tabs) do
        ...
        -- Active tab gradients must render against a neutral white base. A UIGradient
        -- is multiplied by BackgroundColor3, so assigning AccentSoft here darkens the
        -- gradient and makes the first click look different from a refresh/second click.
        if selected and other.RefreshNavGradient then
            other.RefreshNavGradient()
        elseif selected then
            setThemeRole(other.NavButton, "BackgroundColor3", "AccentSoft")
        else
            setThemeRole(other.NavButton, "BackgroundColor3", "Surface3")
        end

        if other.NavGradient then
            other.NavGradient.Enabled = selected
        end
...
function WindowMethods:_setActiveTab(tab)
    ...
    for _, other in ipairs(self.Tabs) do
        ...
        -- Only animate transparency. Tweening BackgroundColor3 fights the UIGradient's
        -- white multiplication base and was the cause of the first-click/double-click
        -- tab gradient bug. Refreshing the selected gradient also guarantees exactly one
        -- active gradient state instead of allowing stale styling to carry over.
...
function WindowMethods:_buildSettings(tab)
    if not tab or tab._premiumSettings then return end
    tab._premiumSettings = true

    local previousSection = tab._currentSection
    local previousColumn = tab._nextColumn

    -- Core settings that belong outside of visual personalization. Modules can add
    -- their own hub/audio cards to this page; this shared block supplies notification
    -- behavior consistently and avoids duplicating it inside Personalization.
    local notifications = tab:CreateSection({
        Name = "Notifications",
...
function WindowMethods:_buildPersonalization(tab)
    ...
    -- Prefer a dedicated Settings tab when one exists. This keeps notification
    -- behavior out of Personalization while remaining backwards compatible with
    -- modules that have not added a Settings page yet.
    local settingsTarget = nil
    for _, candidate in ipairs(self.Tabs or {}) do
        if tostring(candidate.Name or ""):lower() == "settings" then
            settingsTarget = candidate
            break
        end
    end
    self:_buildSettings(settingsTarget or tab)
...
function WindowMethods:CreateTab(name, image)
    ...
    nav.MouseButton1Click:Connect(function()
        ...
        if lowered == "personalization" or lowered == "appearance" then
            self:_buildPersonalization(tab)
        elseif lowered == "settings" then
            self:_buildSettings(tab)
        end
...
local function statusDetailText(window, entry)
    ...
    if #metadata > 0 then
        table.insert(lines, table.concat(metadata, "  Â·  "))
    end
...
function SectionMethods:CreateStatusCard(data)
    ...
    window:AttachTooltip(statusHoverTarget, function()
        ...
        Title = function()
            local entry = object.Entry or {}
            local definition = window:_resolveStatusState(statusEntryState(entry))
            return statusEntryTitle(entry) .. "  Â·  " .. tostring(entry.Label or entry.label or definition.Label)
        end,
        Color = function()
...
function WindowMethods:_refreshOwnerStatusCards()
    ...
    if self.OwnerDashboard.RefreshLabel then
        ...
        self.OwnerDashboard.RefreshLabel:Set(
            source .. "  Â·  " .. tostring(#entries) .. " modules  Â·  refreshes every "
...
function WindowMethods:BuildOwnerDashboard()
    ...
    local username = makeText(
        profileRow,
        "@" .. tostring(self.OwnerInfo.Username or "owner") .. "  Â·  " .. tostring(self.OwnerInfo.UserId or "unknown"),
        11,
        Theme.Muted,
        Enum.Font.Gotham
    )
