    ...
    for _, entry in ipairs(entries) do
        if not favoriteMode or self._favorites[entry.Id] then
            ...
            if total <= (self._quickLimit or 40) then
                shown = shown + 1
                local card = create("Frame", {Parent = self.QuickList, BackgroundColor3 = Theme.Surface2,
                    BorderSizePixel = 0, Size = UDim2.new(1, -6, 0, favoriteMode and 100 or 66), LayoutOrder = shown})
                corner(card, 8)
                if favoriteMode then
                    ...
                    local function resize() card.Size = UDim2.new(1, -6, 0, row.Size.Y.Offset + 30) end
...
function WindowMethods:_setupQuickNavigation()
    ...
    local search = create("TextBox", {Parent = box, BackgroundTransparency = 1, Text = "", PlaceholderText = "Search...",
        ...
        Position = UDim2.fromOffset(7, 0), Size = UDim2.new(1, -34, 1, 0)})
    self.SidebarSearch = search
    local favorite = create("TextButton", {Parent = box, BackgroundTransparency = 1, Text = "\226\152\134",
        TextColor3 = Color3.fromRGB(245, 194, 76), TextSize = 22,
        Position = UDim2.new(1, -27, 0, 0), Size = UDim2.fromOffset(27, 32), AutoButtonColor = false})
    ...
    local function layout()
        ...
        box.Size = UDim2.new(1, -18, 0, collapsed and 64 or 32)
        favorite.Position = collapsed and UDim2.fromOffset(0, 32) or UDim2.new(1, -27, 0, 0)
        self.NavList.Position = UDim2.fromOffset(9, collapsed and 84 or 52)
        local reserve = collapsed and 192 or 160
        local desired = UDim2.new(1, -18, 1, -reserve)
