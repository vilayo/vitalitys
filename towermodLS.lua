...
return function(context)
    ...
    pcall(function()
        -- Settings is a normal visible tab. "diamond" intentionally uses the
        -- library's fallback diamond artwork until the dedicated icon is uploaded.
        local Settings = Window:CreateTab("Settings", "diamond")
        ...
        -- The shared library owns notification settings. Build them into this
        -- Settings tab now so they are present on the first visit rather than only
        -- after the hidden Personalization page has been opened once.
        if type(Window._buildSettings) == "function" then
            Window:_buildSettings(Settings)
        end
    end)
