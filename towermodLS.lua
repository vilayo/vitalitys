-- vitality's hub / The Tower module
-- Production module for the Vitality main loader.
-- Universal migration: Player ESP / Players tools now live in the shared library.
-- Host THIS file at the Tower module URL:
-- https://pastebin.com/raw/LmcTVcDS
--
-- IMPORTANT:
...
return function(context)
    ...
    rawset(_G, "__VITALITY_TOWER_MODULE_BUILD_STATE", {
        Window = Window,
        Ready = false,
    })
    ...
    -- Re-execution safety for Tower's game-specific AFK visual blocker.
    ...
    local TeleportsTab = Window:CreateTab("Teleports", "teleport")
    local ItemVisualsTab = Window:CreateTab("Item ESP", "item-esp")
    ...
    local SectionIcons = {
        ...
        ["Character"] = "character",
        ["Safes"] = "safe",
        ["Advanced Safes"] = "advanced-safe",
        ["Airdrops"] = "airdrop",
        ["Team Healer"] = "heal",
        ["Silent Aim"] = "silent-aim",
        ["Camera Effects"] = "eye",
        ["Loot Teleports"] = "teleport",
        ["Core Item Visuals"] = "item-esp",
        ["Item Filters"] = "items",
    ...
    local function CreateCategorySector(tab, sectionName, side, description)
        local section = tab:CreateSection({
            Name = sectionName,
            Icon = SectionIcons[sectionName] or "info",
            Side = side,
    ...
    pcall(function()
        -- Visible Settings tab. Omitting the icon resolves the tab name through
        -- the library's central semantic registry.
        local Settings = Window:CreateTab("Settings")

        -- Pin Settings to the sidebar footer instead of letting it scroll with
        -- the game-specific tabs. This keeps it directly above the version text,
        -- with a dedicated divider immediately above the Settings button.
        -- The Settings icon is omitted so CreateTab resolves its name through
        -- the library's central semantic registry.
        do
            local settingsButton = Settings.NavButton
            local navList = Window.NavList
            local navPanel = navList and navList.Parent

            if settingsButton and navList and navPanel then
                -- Reserve footer space: divider + Settings button + the existing
                -- fixed version/footer labels that the library draws below them.
                ...
                dividerHolder.Name = "VitalitySettingsDivider"
                dividerHolder.Parent = navPanel
                dividerHolder.BackgroundTransparency = 1
                dividerHolder.BorderSizePixel = 0
                dividerHolder.Position = UDim2.new(0, 9, 1, -113)
                dividerHolder.Size = UDim2.new(1, -18, 0, 10)
                dividerHolder.ZIndex = 1

                local dividerLine = Instance.new("Frame")
                dividerLine.Name = "Line"
                dividerLine.Parent = dividerHolder
                dividerLine.AnchorPoint = Vector2.new(0.5, 0.5)
                dividerLine.Position = UDim2.fromScale(0.5, 0.5)
                dividerLine.Size = UDim2.new(1, -16, 0, 1)
                dividerLine.BackgroundColor3 = Theme.BorderSoft
                dividerLine.BackgroundTransparency = 0.22
                dividerLine.BorderSizePixel = 0
                dividerLine.ZIndex = 1

                if type(Window._registerThemeRenderer) == "function" then
                    Window:_registerThemeRenderer(function()
                        if dividerLine and dividerLine.Parent then
                            dividerLine.BackgroundColor3 = Theme.BorderSoft
                        end
                    end)
                end
        ...
        Personalization._VitalityNotificationsMoved = true
        ...
        local BorderSection = Personalization:CreateSection({
            Name = "Window Border", Icon = "border",
        ...
        -- HUB SETTINGS ---------------------------------------------------------
        local HubSection = Settings:CreateSection({
            Name = "vitality's hub", Icon = "home",
        ...
        -- NOTIFICATIONS ---------------------------------------------------------
        local NotificationsSection = Settings:CreateSection({
        ...
        NotificationsSection:CreateToggle({
            ...
            CurrentValue = Window:GetNotificationsEnabled(),
        })
        ...
        NotificationsSection:CreateSlider({
            ...
            Increment = 0.5,
            Suffix = "s",
            CurrentValue = (Window.NotificationSettings and Window.NotificationSettings.Duration) or 4,
            Callback = function(value)
                if Window.NotificationSettings then
                    Window.NotificationSettings.Duration = value
                end
                if Window._saveConfig then Window:_saveConfig() end
            end,
        })
        ...
        NotificationsSection:CreateDropdown({
            ...
            CurrentOption = (Window.NotificationSettings and Window.NotificationSettings.Position) or "Top right",
            Callback = function(value)
                if Window.NotificationSettings then
                    Window.NotificationSettings.Position = value
                end
                if Window._saveConfig then Window:_saveConfig() end
            end,
        })
        ...
        NotificationsSection:CreateSlider({
            ...
            Increment = 1,
            Suffix = "%",
            CurrentValue = (Window.NotificationSettings and Window.NotificationSettings.Opacity) or 100,
            Callback = function(value)
                if Window.NotificationSettings then
                    Window.NotificationSettings.Opacity = value
                end
                if Window._saveConfig then Window:_saveConfig() end
            end,
        })
    ...
    -- Player ESP is now provided by the library-level Universal > Visuals page.
    -- Tower keeps only game-specific Item ESP in this module.

    -- Generic cursor controls and player teleport tools now live in
    -- Universal > Utilities so they are available in every supported game.
