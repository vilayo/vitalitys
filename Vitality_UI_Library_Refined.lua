...
local function applyButtonGradient(guiObject)
    local gradient = create("UIGradient", {
        ...
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, (Library._activeWindow and Library._activeWindow.ButtonGradientStart) or Theme.Button),
            ColorSequenceKeypoint.new(1, (Library._activeWindow and Library._activeWindow.ButtonGradientEnd) or Theme.ButtonHover),
        }),
        Rotation = 0,
        Parent = guiObject,
    })
...
local UIStyle = {
    WindowWidth = 760, WindowHeight = 500, SidebarWidth = 124, TopbarHeight = 42,
    CardRadius = 10, CardGap = 10, CardInset = 13,
    HeaderHeight = 44, HeaderHeightWithSubtitle = 56,
    ...
    RowInset = 13, RowGap = 14, RowMinHeight = 48, StatusRowHeight = 34,
    RowVerticalPadding = 9, ControlHeight = 32,
    ButtonWidth = 112, FieldWidth = 154, ToggleWidth = 48, ToggleHeight = 26,
    DividerInset = 13, SectionGap = 10, ResponsiveBreakpoint = 570,
}
...
local function applySoftSurfaceGradient(guiObject, window, topRole, bottomRole, rotation)
    ...
    local function refresh()
        local top = (window and window.SurfaceGradientStart) or Theme[topRole or "Surface3"] or Theme.Surface3
        local bottom = (window and window.SurfaceGradientEnd) or Theme[bottomRole or "Surface2"] or Theme.Surface2
...
local NotificationTypes = {
    ...
    Info = {Role = "AccentVisible", Icon = "i", Sound = "Info"},
}
...
function Library:Notify(data)
    ...
    local icon = makeText(card, semantic.Icon == "Check" and "" or semantic.Icon, 14, Theme.Background, Enum.Font.GothamBold, Enum.TextXAlignment.Center)
    ...
    icon.ZIndex = 302
    corner(icon, 15)
    if semantic.Icon == "Check" then createCheckmark(icon, Theme.Background, 304) end
    ...
    local targetHeight = math.max(64, 47 + body.TextBounds.Y)
    if window then
        window:_tween(card, 0.24, {GroupTransparency = 0, Size = UDim2.new(1, 0, 0, targetHeight)})
        window:_tween(cardScale, 0.30, {Scale = 1})
    else
        tween(card, 0.24, {GroupTransparency = 0, Size = UDim2.new(1, 0, 0, targetHeight)})
        ...
        for _, object in ipairs(self.Gui:GetDescendants()) do
            ...
            if object:IsA("UIGradient") and object.Name == "NovaButtonGradient" then
                object.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, self.ButtonGradientStart or Theme.Button),
                    ColorSequenceKeypoint.new(1, self.ButtonGradientEnd or Theme.ButtonHover),
                })
            end
        end
    end
...
function WindowMethods:SetGradient(kind, startColor, endColor)
    if typeof(startColor) ~= "Color3" or typeof(endColor) ~= "Color3" then return false end
    kind = tostring(kind or ""):lower()
    if kind == "button" then
        self.ButtonGradientStart, self.ButtonGradientEnd = startColor, endColor
    elseif kind == "accent" then
        self.AccentGradientStart, self.AccentGradientEnd = startColor, endColor
        self:SetThemeAccent(startColor)
    elseif kind == "surface" or kind == "card" then
        self.SurfaceGradientStart, self.SurfaceGradientEnd = startColor, endColor
    else
        return false
    end
...
function WindowMethods:GetGradient(kind)
    kind = tostring(kind or ""):lower()
    if kind == "button" then return self.ButtonGradientStart or Theme.Button, self.ButtonGradientEnd or Theme.ButtonHover end
    if kind == "accent" then return self.AccentGradientStart or Theme.Accent, self.AccentGradientEnd or Theme.AccentDark end
    if kind == "surface" or kind == "card" then return self.SurfaceGradientStart or Theme.Surface3, self.SurfaceGradientEnd or Theme.Surface2 end
end

function WindowMethods:ResetGradients()
    self.ButtonGradientStart, self.ButtonGradientEnd = nil, nil
    self.AccentGradientStart, self.AccentGradientEnd = nil, nil
    self.SurfaceGradientStart, self.SurfaceGradientEnd = nil, nil
    return self:SetTheme(self.ThemeName or "Dark")
end
...
function TabMethods:CreateColorPicker(data)
    ...
    if gradientKind then
        ...
        roleDefault = gradientStop == "end" and gradientEnd or gradientStart
    end
...
function Library:CreateWindow(settings)
    ...
    logo.Position=UDim2.fromOffset(13,11); logo.Size=UDim2.fromOffset(20,20)
    ...
    local statusChip=create("Frame",{
        Parent=topbar,AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-102,0.5,0),Size=UDim2.fromOffset(162,28),
    ...
    local minimize=create("TextButton",{
        Parent=topbar,AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-40,0.5,0),Size=UDim2.fromOffset(24,24),
    ...
    local close=create("TextButton",{
        Parent=topbar,AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-12,0.5,0),Size=UDim2.fromOffset(24,24),
    ...
    local function updateScale()
        ...
        uiScale.Scale=math.clamp(math.min(vp.X/(UIStyle.WindowWidth + 40),vp.Y/(UIStyle.WindowHeight + 60)),0.86,1)
    end
    updateScale()
