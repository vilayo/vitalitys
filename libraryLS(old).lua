...
function WindowMethods:AttachTooltip(target, content, options)
    ...
    local function show()
        ...
        local below = anchorPosition.Y + anchorSize.Y + 8
        local y = below + height <= viewport.Y - 8
            and below
            or math.max(8, anchorPosition.Y - height - 8)
...
local function statusDetailText(window, entry)
    ...
    local lines = {
        tostring(entry.Label or entry.label or definition.Label),
    }
    ...
    if summary and tostring(summary) ~= "" then table.insert(lines, tostring(summary)) end
    if detail and tostring(detail) ~= "" and tostring(detail) ~= tostring(summary) then
        table.insert(lines, tostring(detail))
    end
    if tooltip and tostring(tooltip) ~= "" and tostring(tooltip) ~= tostring(detail) then
    ...
    if version and tostring(version) ~= "" then table.insert(metadata, "Version " .. tostring(version)) end
    if updatedAt and tostring(updatedAt) ~= "" then table.insert(metadata, "Updated " .. tostring(updatedAt)) end
    if #metadata > 0 then table.insert(lines, table.concat(metadata, "  |  ")) end
...
function SectionMethods:CreateStatusCard(data)
    ...
    nameLabel.Size = UDim2.new(1, -154, 0, 19)
    ...
    statusLabel.Size = UDim2.fromOffset(102, 19)
    ...
    window:AttachTooltip(card, function()
        return statusDetailText(window, object.Entry)
    end, {
        Title = function()
            local entry = object.Entry or {}
            local definition = window:_resolveStatusState(statusEntryState(entry))
            return statusEntryTitle(entry) .. "  ·  " .. tostring(entry.Label or entry.label or definition.Label)
        end,
        Color = function()
...
function WindowMethods:_refreshOwnerStatusCards()
    ...
    if self.OwnerDashboard.RefreshLabel then
        ...
        self.OwnerDashboard.RefreshLabel:Set(
            source .. "  ·  " .. tostring(#entries) .. " modules  ·  refreshes every "
...
function WindowMethods:BuildOwnerDashboard()
    ...
    local tab = self:CreateTab("Owner", "home")
    ...
    local username = makeText(
        profileRow,
        "@" .. tostring(self.OwnerInfo.Username or "owner") .. "  ·  " .. tostring(self.OwnerInfo.UserId or "unknown"),
        11,
        Theme.Muted,
        Enum.Font.Gotham
    )
...
function Library:CreateWindow(settings)
    ...
    window:AttachTooltip(statusChip, function()
        ...
        Title = function()
            local entry = window.CurrentStatusEntry or {}
            return statusEntryTitle(entry) .. " status"
        end,
        Color = function()
