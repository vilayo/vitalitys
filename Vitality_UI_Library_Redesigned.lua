...
local function applySoftSurfaceGradient(guiObject, window, topRole, bottomRole, rotation)
    ...
    local function refresh()
        local top = Theme[topRole or "Surface3"] or Theme.Surface3
        local bottom = Theme[bottomRole or "Surface2"] or Theme.Surface2
...
function Library:Notify(data)
    ...
    local icon = makeText(card, data.Success == false and "!" or "âœ“", 16, Theme.Background, Enum.Font.GothamBold, Enum.TextXAlignment.Center)
    ...
    local close = makeText(card, "Ã—", 22, Theme.Text, Enum.Font.Gotham, Enum.TextXAlignment.Center)
...
function WindowMethods:SetVisible(visible, immediate)
    ...
    if visible then
        ...
        main.Size =
            self._minimized
            and (self._collapsedSize or UDim2.fromOffset(1120, 52))
            or (self._expandedSize or UDim2.fromOffset(1120, 680))
...
function WindowMethods:SetMinimized(minimized, immediate)
    ...
    local expanded =
        self._expandedSize
        or UDim2.fromOffset(1120, 680)

    local collapsed =
        self._collapsedSize
        or UDim2.fromOffset(1120, 52)
...
function WindowMethods:CreateTab(name, image)
    ...
    corner(nav, 9)
    local navStroke = stroke(nav, Theme.AccentVisible, 1, 0.52)
    ...
    local function recalc()
        ...
        local singleColumn = available > 0 and available < 760
...
function TabMethods:CreateSection(name, subtitleOrTitleOnly, column)
    ...
    local headerHeight
    if subtitle then
        headerHeight = hasIcon and 88 or 78
    elseif titleOnly then
        headerHeight = hasIcon and 68 or 56
    else
        headerHeight = hasIcon and 76 or 62
    end
    ...
    if hasIcon then
        ...
        applySoftSurfaceGradient(iconHolder, self.Window, "Surface3", "Surface2", 128)
        ...
        headingX = UIStyle.CardInset + UIStyle.HeaderIconSize + 18
    end
...
local function rowText(row, name, info, rightWidth)
    local right = tonumber(rightWidth) or UIStyle.FieldWidth
    local textWidthOffset = UIStyle.RowInset + UIStyle.RowInset + UIStyle.RowGap + right

    local textArea = create("Frame", {
        Parent = row,
        AnchorPoint = Vector2.new(0, 0.5),
        Position = UDim2.new(0, UIStyle.RowInset, 0.5, 0),
        Size = UDim2.new(1, -textWidthOffset, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        BackgroundTransparency = 1,
    })
    ...
    create("UIListLayout", {
        ...
        SortOrder = Enum.SortOrder.LayoutOrder,
        Padding = UDim.new(0, info and 4 or 0),
    })

    local nameLabel = makeText(textArea, name or "Control", UIStyle.LabelSize, Theme.Text, Enum.Font.GothamMedium)
    ...
    if info and tostring(info) ~= "" then
        infoLabel = makeText(textArea, tostring(info), UIStyle.InfoSize, Theme.Muted, Enum.Font.Gotham)
    ...
    local function refreshHeight()
        task.defer(function()
            if not row.Parent or not textArea.Parent then return end
            local base = tonumber(row:GetAttribute("NovaRowBaseHeight")) or UIStyle.RowMinHeight
            local textHeight = math.max(textArea.AbsoluteSize.Y, UIStyle.LabelSize + 4)
            local wanted = math.max(base, textHeight + UIStyle.RowVerticalPadding * 2)
            if math.abs(row.Size.Y.Offset - wanted) > 0.5 then
                row.Size = UDim2.new(1, 0, 0, wanted)
            end
        end)
    end

    textArea:GetPropertyChangedSignal("AbsoluteSize"):Connect(refreshHeight)
    row:GetPropertyChangedSignal("AbsoluteSize"):Connect(refreshHeight)
    nameLabel:GetPropertyChangedSignal("TextBounds"):Connect(refreshHeight)
    if infoLabel then infoLabel:GetPropertyChangedSignal("TextBounds"):Connect(refreshHeight) end
    refreshHeight()

    return nameLabel, infoLabel, textArea
end
...
function TabMethods:CreateButton(data)
    ...
    local row = section:_row(70)
    rowText(row, data.Name or "Button", data.Info, UIStyle.ButtonWidth)
    ...
    button.MouseButton1Click:Connect(function()
        ...
        window:_tween(button, 0.07, {Size = UDim2.fromOffset(UIStyle.ButtonWidth - 4, UIStyle.ControlHeight - 2)})
        task.delay(window:_motionDuration(0.08), function()
            if button.Parent then
                window:_tween(button, 0.10, {Size = UDim2.fromOffset(UIStyle.ButtonWidth, UIStyle.ControlHeight)})
            end
        end)
...
function TabMethods:CreateToggle(data)
    ...
    local section = self:_sectionFrom(data)
    local row = section:_row(70)
    rowText(row, data.Name or "Toggle", data.Info, 72)
    ...
    local function render(animate)
        ...
        local knobProps = {Position = object.Value and UDim2.fromOffset(30, 4) or UDim2.fromOffset(4, 4)}
...
function TabMethods:CreateSlider(data)
    ...
    local window = self.Window
    local section = self:_sectionFrom(data)
    local row = section:_row(74)
    rowText(row, data.Name or "Slider", data.Info, 236)
...
function TabMethods:CreateColorPicker(data)
    ...
    hexLabel.Position = UDim2.fromOffset(49, 0)
...
local function buildKeySystem(window)
    ...
    if settings.GetKeyURL or settings.GetKeyCallback then
        local getKey=makeText(card,"Get key  â†—",10,Theme.AccentVisible,Enum.Font.GothamMedium,Enum.TextXAlignment.Right)
    ...
    local function rejectKey(message)
        ...
        errorText.Text = tostring(message or "â—  Invalid key")
    ...
    local function acceptKey(candidate, session, info, shouldSaveForReuse)
        ...
        local acceptedText = "âœ“  Key accepted"
        if session.ExpiresAt then
            local remaining = math.max(0, session.ExpiresAt - os.time())
            acceptedText = "âœ“  Key accepted â€¢ " .. formatKeyRemaining(remaining)
        end
    ...
    local function tryRemoteKey()
        ...
        task.spawn(function()
            ...
            if not accepted then
                ...
                rejectKey(message or "â—  Invalid key")
...
function Library:CreateWindow(settings)
    ...
    window._expandedSize = UDim2.fromOffset(1120, 680)
    window._collapsedSize = UDim2.fromOffset(1120, 52)
    ...
    logo.Position=UDim2.fromOffset(17,15); logo.Size=UDim2.fromOffset(22,22)
    ...
    local minimize=create("TextButton",{
        ...
        BackgroundColor3=Theme.Surface3,BorderSizePixel=0,Text="âˆ’",TextColor3=Theme.Text,TextSize=18,Font=Enum.Font.Gotham,AutoButtonColor=false,
    ...
    local close=create("TextButton",{
        ...
        BackgroundColor3=Theme.Surface3,BorderSizePixel=0,Text="Ã—",TextColor3=Theme.Text,TextSize=20,Font=Enum.Font.Gotham,AutoButtonColor=false,
    ...
    local nav=create("CanvasGroup",{
        Parent=bodyViewport,BackgroundColor3=Theme.Surface,GroupTransparency=0,BorderSizePixel=0,Position=UDim2.fromOffset(0,0),Size=UDim2.new(0,158,1,0),
    ...
    local content=create("Frame",{
        Parent=bodyViewport,BackgroundTransparency=1,BorderSizePixel=0,Position=UDim2.fromOffset(158,0),Size=UDim2.new(1,-158,1,-1),
    ...
    local function updateScale()
        ...
        uiScale.Scale=math.clamp(math.min(vp.X/1200,vp.Y/760),0.82,1)
    end
    updateScale()
