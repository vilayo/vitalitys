...
function WindowMethods:_refreshTabStyles()
    ...
    for _, other in ipairs(self.Tabs) do
        ...
        if selected and other.RefreshNavGradient then other.RefreshNavGradient()
        else setThemeRole(other.NavButton, "BackgroundColor3", "Surface3") end
...
function WindowMethods:_appearanceSnapshot()
    local result = {}
    for _, key in ipairs({"ThemeName", "FontPreset", "CustomAccent", "CustomButton", "GradientOptions", "BorderStrokeSettings", "NotificationSettings"}) do
        result[key] = self[key]
    end
    for _, prefix in ipairs({"Accent", "Button", "Tab", "Icon", "Surface"}) do
...
function WindowMethods:_restoreAppearance()
    local saved = self.ConfigData and self.ConfigData._VitalityAppearanceV1
    ...
    for _, key in ipairs({"GradientOptions", "BorderStrokeSettings", "NotificationSettings"}) do
...
function WindowMethods:SetScriptStatus(status, detail)
    ...
    if self.StatusHalo then setThemeRole(self.StatusHalo, "BackgroundColor3", state.Role) end
...
function WindowMethods:CreateTab(name, image)
    ...
    local function fitNavigation()
        ...
        local width = math.max(40, nav.AbsoluteSize.X / logicalScale(nav) - 42)
        nav.Size = UDim2.new(1, 0, 0, math.max(40, math.ceil(textSize(title, width).Y) + 16))
        icon.AnchorPoint = Vector2.new(0, 0.5)
        icon.Position = UDim2.new(0, 10, 0.5, 0)
    end
    nav:GetPropertyChangedSignal("AbsoluteSize"):Connect(fitNavigation)
    title:GetPropertyChangedSignal("Font"):Connect(fitNavigation)
