...
function WindowMethods:_setupCommandBar()
    ...
    local root = create("CanvasGroup", {
        ...
        Size = UDim2.fromOffset(width, 48 + 8 + (maxSuggestions * (rowHeight + rowGap)) + 12),
        BackgroundTransparency = 1,
        GroupTransparency = 1,
        BorderSizePixel = 0,
        Visible = false,
    ...
    prompt.Size = UDim2.fromOffset(22, 48)
    prompt.ZIndex = 603
    ...
    local input = create("TextBox", {
        ...
        MultiLine = false,
        Text = "",
        PlaceholderText = tostring(options.Placeholder or "Type a command..."),
        TextColor3 = Theme.Text,
        PlaceholderColor3 = Theme.Muted,
        TextSize = 14,
    ...
    for index = 1, maxSuggestions do
        ...
        descLabel.Position = UDim2.fromOffset(12, 21)
        ...
        self:_trackConnection(row.MouseButton1Click:Connect(function()
            local command = self._commandMatches and self._commandMatches[index]
            if not command then return end
            local currentText = input.Text
            local argsText = currentText:match("^%s*[^%s]+%s+(.+)$")
            input.Text = command.Name .. (argsText and (" " .. argsText) or "")
            self._commandSelection = index
    ...
    local function renderRows()
        ...
        for index, rowData in ipairs(self.CommandBarRows) do
            local command = matches[index]
            rowData.Button.Visible = command ~= nil
            if command then
                rowData.Name.Text = command.Name
                local detail = command.Description ~= "" and command.Description or ("Usage: " .. command.Usage)
                rowData.Description.Text = detail
    ...
    function self:_refreshCommandSuggestions()
        ...
        self._commandMatches = self:GetCommandMatches(self.CommandBarInput.Text, maxSuggestions)
    ...
    self:_trackConnection(input.FocusLost:Connect(function(enterPressed)
        ...
        if enterPressed then
            ...
            local pieces = splitCommandLine(text)
            local typedName = pieces[1]
            if typedName and not self:_findCommand(typedName) then
                local selected = self._commandMatches and self._commandMatches[self._commandSelection or 1]
                if selected then
                    local argsText = text:match("^%s*[^%s]+%s+(.+)$")
                    text = selected.Name .. (argsText and (" " .. argsText) or "")
                end
            end
            local ok, err = self:ExecuteCommand(text)
            if ok then
                self:_playSound("Confirm")
                self:CloseCommandBar(false, true)
            else
            ...
            -- Clicking a suggestion briefly removes TextBox focus before the
            -- suggestion button receives MouseButton1Click. Defer one scheduler
            -- step and only close when the pointer is actually outside the whole
            -- command palette. This gives the bar natural click-away behavior
            -- without breaking suggestion selection.
            task.defer(function()
                if not self._commandBarOpen or not root.Parent then return end
                local pointer = UserInputService:GetMouseLocation()
                if not pointInside(root, pointer) then
                    self:CloseCommandBar(false, true)
                end
            end)
        end
    ...
    self:_trackConnection(UserInputService.InputBegan:Connect(function(key, processed)
        ...
        if self._commandBarOpen then
            ...
            elseif key.KeyCode == Enum.KeyCode.Tab then
                local selected = self._commandMatches and self._commandMatches[self._commandSelection or 1]
                if selected then
                    local argsText = input.Text:match("^%s*[^%s]+%s+(.+)$")
                    input.Text = selected.Name .. (argsText and (" " .. argsText) or " ")
