...
-- Static fallback modules: The Tower, Apocalypse Rising 2, Murder Mystery 2, Flick, KAT, Funky Friday. Cloudflare can add/remove live modules dynamically.
...
-- Funky Friday production module.
local FUNKY_FRIDAY_MODULE_URL = "https://vitalitys.lol/funkyfridaymodLS.lua"
...
-- Funky Friday: observed place 6447798030; verified universe/game.GameId.
local FUNKY_FRIDAY_GAME_ID = 2404080894
...
local KEYAUTH = {
    ...
    -- Bump this only when the client identity scheme intentionally changes.
    -- libraryLS uses it to invalidate stale locally-saved key sessions once.
    IdentityVersion = 2,

    IdentityFolder = "VitalityHubKeys",
    IdentityFile = "VitalityDeviceId.txt",

    -- Very long subscriptions still obey KeyAuth revocation/expiry, but are
    -- displayed and reused like a lifetime license.
    LongTermSeconds = 5 * 365 * 24 * 60 * 60,
}
...
local function stableIdentityHash(value)
    -- This is only an obfuscating/stability hash, not a cryptographic primitive.
    -- KeyAuth needs a stable identifier, not the executor's raw hardware string.
    ...
    for i = 1, #value do
        ...
        h1 = (h1 * 33 + byte) % mod
        h2 = (h2 * 65599 + byte) % mod
    end

    return string.format("%08x%08x", h1, h2)
end

local function resolveExecutorHWID()
    local candidates = {}

    if type(gethwid) == "function" then
        table.insert(candidates, {"gethwid", gethwid})
    end

    if type(get_hwid) == "function" then
        table.insert(candidates, {"get_hwid", get_hwid})
    end

    if type(getexecutorhwid) == "function" then
        table.insert(candidates, {"getexecutorhwid", getexecutorhwid})
    end

    if syn and type(syn.gethwid) == "function" then
        table.insert(candidates, {"syn.gethwid", syn.gethwid})
    end

    for _, candidate in ipairs(candidates) do
        local ok, result = pcall(candidate[2])
        result = ok and trimString(result) or ""
...
local function ensureIdentityFolder()
    if type(makefolder) ~= "function" then return end
    pcall(function()
        if type(isfolder) ~= "function" or not isfolder(KEYAUTH.IdentityFolder) then
            makefolder(KEYAUTH.IdentityFolder)
        end
    end)
end

local function getPersistedInstallIdentity()
    if type(readfile) ~= "function" or type(writefile) ~= "function" then
        return nil
    end

    ensureIdentityFolder()
    local path = KEYAUTH.IdentityFolder .. "/" .. KEYAUTH.IdentityFile
    ...
    stored = ok and trimString(stored) or ""
    if stored ~= "" and #stored >= 8 then
        return stored
    end

    local generated = "install-" .. HttpService:GenerateGUID(false)
    local writeOk = pcall(writefile, path, generated)
    return writeOk and generated or nil
end

local function getKeyAuthIdentity()
    -- Prefer a real executor HWID when the executor exposes one, but send only
    -- a stable derived identifier to KeyAuth instead of the raw hardware value.
    local rawHWID, source = resolveExecutorHWID()
    if rawHWID then
        return "vitality-v2-hwid-" .. stableIdentityHash(rawHWID), source
    end

    -- A persisted installation ID is the next-best choice. It stays stable
    -- across executions without tying the license directly to a Roblox account.
    local installId = getPersistedInstallIdentity()
    if installId then
        return "vitality-v2-install-" .. stableIdentityHash(installId), "local-install-id"
    end

    -- Last-resort compatibility fallback for executors without HWID or file APIs.
    local player = Players.LocalPlayer
    return player
        and ("vitality-v2-user-" .. tostring(player.UserId))
        or "vitality-v2-user-unknown",
        "roblox-user-fallback"
end

local function normalizeUnixTimestamp(value)
    local number = tonumber(value)
    if not number then return nil end

    -- Accept either Unix seconds or an accidental millisecond representation.
...
local function normalizeTimeLeft(value)
    local number = tonumber(value)
    if not number then return nil end
    number = math.floor(number)
    if number < 0 then return nil end
...
local function subscriptionTiming(subscription, now)
    if type(subscription) ~= "table" then
        return nil, nil, nil
    end

    local absolute = normalizeUnixTimestamp(subscription.expiry)
    local timeLeft = normalizeTimeLeft(subscription.timeleft)
    local derived = timeLeft and (now + timeLeft) or nil

    -- KeyAuth documents expiry as an absolute Unix timestamp and timeleft as
    -- remaining seconds. Never let a zero/stale timeleft overwrite a future
    -- absolute expiry.
    local effective = absolute
    if derived and (not effective or derived > effective) then
        effective = derived
    end

    return effective, absolute, timeLeft
end

local function selectBestSubscription(info)
    ...
    local now = os.time()
    local best, bestEffective, bestAbsolute, bestTimeLeft
    local bestScore = -math.huge

    for _, subscription in ipairs(subscriptions) do
        if type(subscription) == "table" then
            local effective, absolute, timeLeft = subscriptionTiming(subscription, now)
            local score = effective or -1

            -- Prefer the subscription with the furthest valid entitlement.
    ...
    return best, bestEffective, bestAbsolute, bestTimeLeft
end

local function classifyKeyAuthFailure(message)
    local lower = tostring(message or ""):lower()

    local expired =
        lower:find("expir", 1, true) ~= nil

    local hwidMismatch =
        lower:find("hwid", 1, true) ~= nil
        or lower:find("hardware", 1, true) ~= nil

    local banned =
        lower:find("banned", 1, true) ~= nil
        or lower:find("blacklist", 1, true) ~= nil

    local sessionProblem =
        lower:find("session", 1, true) ~= nil
        and (
            lower:find("invalid", 1, true) ~= nil
            or lower:find("not found", 1, true) ~= nil
            or lower:find("expired", 1, true) ~= nil
        )
...
local function performKeyAuthLicenseRequest(licenseKey, identity)
...
local function validateKeyAuthLicense(licenseKey)
    licenseKey = trimString(licenseKey)
    if licenseKey == "" then
        return false, {
            Provider = "KeyAuth",
    ...
    local identity, identitySource = getKeyAuthIdentity()
    local response, requestError = performKeyAuthLicenseRequest(licenseKey, identity)
    ...
    if not response then
        return false, {
            ...
            IdentityVersion = KEYAUTH.IdentityVersion,
        }
    end

    if response.success ~= true then
        local message = tostring(response.message or "Invalid license.")
        local classification = classifyKeyAuthFailure(message)

        -- A stale KeyAuth API session is recoverable. Reinitialize once before
        -- treating the license itself as invalid.
        if classification.SessionProblem then
            KEYAUTH.SessionId = nil

            local retryInit = initializeKeyAuth()
            if retryInit then
                response, requestError = performKeyAuthLicenseRequest(licenseKey, identity)
            end

            if not response then
                return false, {
                    Provider = "KeyAuth",
                    Message = requestError or "Could not validate license after refreshing the KeyAuth session.",
                    Retryable = true,
                    Transient = true,
                    IdentitySource = identitySource,
                    IdentityVersion = KEYAUTH.IdentityVersion,
            ...
            if response.success ~= true then
                message = tostring(response.message or "Invalid license.")
                classification = classifyKeyAuthFailure(message)
            end
        end

        if response.success ~= true then
            local friendly = message
            if classification.Expired then
                friendly = "this key has expired and is not usable!"
            elseif classification.HWIDMismatch then
                friendly = "HWID mismatch. Reset this user in KeyAuth, then authenticate again."
            end
            ...
            return false, {
                ...
                HWIDMismatch = classification.HWIDMismatch,
    ...
    local subscription, effectiveExpiry, absoluteExpiry, timeLeft =
        selectBestSubscription(info)

    local now = os.time()
    local expiryReliable = effectiveExpiry ~= nil and effectiveExpiry > now

    -- A successful server login is authoritative. If KeyAuth returns a stale
    -- or zero timing field, do NOT convert that into an immediate local expiry.
    local expiresAt = expiryReliable and effectiveExpiry or nil
    ...
    local remaining =
        expiresAt and math.max(0, expiresAt - now) or nil

    local lifetimeLike =
        permanent
        or (remaining ~= nil and remaining >= KEYAUTH.LongTermSeconds)
    ...
    return true, {
        ...
        ExpiryReliable = expiryReliable,
        IdentitySource = identitySource,
        IdentityVersion = KEYAUTH.IdentityVersion,
        ServerHWIDPresent = trimString(info.hwid) ~= "",
    }
end
...
local SupportedGames = {
    ...
    [FUNKY_FRIDAY_GAME_ID] = {
        ModuleId = "funky-friday",
        LiveModuleId = "funky-friday",
        Name = "Funky Friday",
        Version = "1.0.0-LS",
        Module = FUNKY_FRIDAY_MODULE_URL,
        Status = "yellow",
        State = "testing",
        Label = "In Testing",
    },
...
function DevWorkspace:_renderTabs()
    ...
    for _, item in ipairs(workspace.Tabs) do
        local button = devMake("TextButton", {
            ...
            Text = item.Name:lower() == "main.lua" and "main.lua  â€¢  PREVIEW" or item.Name,
...
function DevWorkspace:_buildGui(window)
    ...
    local statusLabel = devMake("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "GameId " .. tostring(game.GameId) .. "  â€¢  paged editor",
...
local Window = NovaField:CreateWindow({
        ...
        OnUnsupported = function(context)
            ...
            Section:CreateParagraph({
                ...
                Content = "GameId: " .. tostring(context.GameId)
                    .. "  ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢  PlaceId: " .. tostring(context.PlaceId),
            })
    ...
    KeySettings = {
        ...
        SaveKey = true, -- saved locally; always revalidated with KeyAuth before reuse

        -- Revalidate remotely without treating transient network/API failures as
        -- an expired license. The local absolute expiry countdown still runs.
        RemoteRefreshInterval = 30,
        RemoteFailureGraceCount = 3,
        RemoteFailureGraceSeconds = 120,

        -- Remember successfully validated remote keys locally and revalidate
        -- them with KeyAuth on EVERY fresh execution before unlocking the hub.
        -- This avoids repeated typing while still respecting expiry/revocation/
        -- HWID changes. The cached key by itself can never unlock Vitality.
        AutoReuseRemoteKeys = true,
        AutoReuseTemporaryKeys = true,
        AutoReuseLongTermKeys = true,
        LongTermThresholdSeconds = KEYAUTH.LongTermSeconds,

        -- Version 2 intentionally invalidates the old locally cached sessions so
        -- this deployment prompts once for a new key after the HWID migration.
        IdentityVersion = KEYAUTH.IdentityVersion,
        ResetLegacyKeyCache = true,
