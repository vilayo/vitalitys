...
function DevWorkspace:RunActive(window)
    ...
    local context
    local temporaryRuntime = false
    if self.Runtime and self.Runtime.Context then
        context = self.Runtime.Context
    else
        local previewWindow = self:_createPreviewWindow()
        if not previewWindow then return false end
        context = devCreateContext(previewWindow)
        temporaryRuntime = true
    end

    local ok, result = pcall(chunk, context)
    if not ok then
        if temporaryRuntime then
            context:Destroy()
            self:_destroyPreviewWindow()
        end
    ...
    local returned = result
    if type(returned) == "function" then
        local okStart, startResult = pcall(returned, context)
        if not okStart then
            if temporaryRuntime then
                context:Destroy()
                self:_destroyPreviewWindow()
            end
            self:_appendOutput("Start error in " .. item.Name .. ": " .. tostring(startResult), "error")
            return false
        end
        returned = startResult
    elseif type(returned) == "table" then
        local startMethod = returned.Start or returned.Load or returned.Init
        if type(startMethod) == "function" then
            local okStart, startError = pcall(startMethod, returned, context)
            if not okStart then
                ...
                self:_appendOutput("Start error in " .. item.Name .. ": " .. tostring(startError), "error")
                return false
            end
        end
    end
    ...
    if temporaryRuntime then
        self.Runtime = {
            ...
            LoadedAt = os.clock(),
            PreviewWindow = self.PreviewWindow,
            ScratchOnly = true,
        }
    end
    ...
    self:_appendOutput("Ran " .. item.Name .. " in the isolated preview.", "success")
    return true
end
...
local function registerLiveOwnerCommands(window)
    ...
    window:RegisterCommand({
        Name = "unloadmodule",
        Aliases = {"devunload"},
        Description = "Close the isolated development preview and clean all tracked resources.",
        Usage = "unloadmodule",
        Category = "Developer",
        Callback = function()
