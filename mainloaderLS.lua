...
-- The editor has no artificial source truncation. Its layout path is optimized
-- around a practical target of roughly 50,000 unwrapped Lua lines. Actual
-- responsiveness still depends on Roblox/executor TextBox performance.
local DEV_EDITOR_TARGET_LINES = 50000
local DEV_EDITOR_LINE_HEIGHT = 18
local DEV_EDITOR_CHAR_WIDTH = 8.4
local DEV_EDITOR_MAX_CANVAS_WIDTH = 200000
...
function DevWorkspace:_deleteActiveTab()
    ...
    if self.Editor then
        self.Editor.Text = workspace.Tabs[1].Code
        self.Editor.CursorPosition = #self.Editor.Text + 1
    end
    self:_renderTabs()
    self:_saveWorkspace(true)
...
function DevWorkspace:_refreshEditorCanvas()
    ...
    -- Debounce layout work. This avoids repeatedly measuring multi-megabyte
    -- source while the user is typing quickly.
    ...
    task.defer(function()
        ...
        local text = self.Editor.Text or ""
        local lineCount, longestLine = devMeasureEditorText(text)

        local viewportWidth = math.max(300, self.EditorScroll.AbsoluteSize.X - 24)
        local viewportHeight = math.max(260, self.EditorScroll.AbsoluteSize.Y - 24)
        ...
        local measuredHeight = math.max(
            viewportHeight,
            math.ceil(lineCount * DEV_EDITOR_LINE_HEIGHT + 32)
        )
        ...
        if lineCount > DEV_EDITOR_TARGET_LINES and not self._largeEditorWarned then
            self._largeEditorWarned = true
            self:_appendOutput(
                "Editor source is above the 50,000-line performance target. It is not truncated, but Roblox/executor UI performance may vary.",
                "warn"
            )
        elseif lineCount <= DEV_EDITOR_TARGET_LINES then
            self._largeEditorWarned = false
        end
    end)
end
...
function DevWorkspace:_buildGui(window)
    ...
    devMake("UIPadding", {Parent = moduleIdInput, PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8)})
    ...
    devMake("UIPadding", {Parent = moduleNameInput, PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8)})

    local registerButton = devMake("TextButton", {
        Parent = moduleBar,
        Position = UDim2.fromOffset(356, 5),
    ...
    local removeButton = devMake("TextButton", {
        Parent = moduleBar,
        Position = UDim2.fromOffset(468, 5),
    ...
    local resetBaseButton = devMake("TextButton", {
        Parent = moduleBar,
        Position = UDim2.fromOffset(567, 5),
    ...
    resetBaseButton.MouseButton1Click:Connect(function()
        ...
        workspace.Active = "main.lua"
        if self.Editor and self.Editor.Parent then self.Editor.Text = main.Code end
        self:_renderTabs()
        self:_saveWorkspace(true)
    ...
    local editor = devMake("TextBox", {
        ...
        RichText = false,
        Text = "",
        PlaceholderText = "-- main.lua = preview module | other tabs = standalone scratch execution",
    ...
    local actionBar = devMake("Frame", {
        Parent = root,
        Position = UDim2.fromOffset(12, 466),
        Size = UDim2.new(1, -24, 0, 38),
        BackgroundTransparency = 1,
    })
    ...
    local output = devMake("TextLabel", {
        Parent = root,
        Position = UDim2.fromOffset(12, 510),
